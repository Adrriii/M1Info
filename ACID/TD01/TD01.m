...
M = randi(9,4,9);
ML = sort(reshape(M,size(M,1)*size(M,2),1),1);
R2 = transpose(reshape(ML,size(M,2),size(M,1)));
...
% 5 : https://fr.mathworks.com/help/matlab/ref/plot.html?searchHighlight=plot&s_tid=doc_srchtitle
% plot: Relie et représente les points sous forme de courbe (2D) ou surface (3D)
% scatter: Représente chaque point par un cercle


% hold on;

X = [1 : 10];
Y = droite2DVD(X, [5 2], [1 1]);
Yn = droite2DNorm(X, [1 2], [1 1]);
% hold on;
% plot(X, Y);
% plot(X, Yn);
